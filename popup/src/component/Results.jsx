...
function QAPanel() {
  ...
  return (
    <span className="ml-1 group relative">
      ...
      <div className="w-80 p-2 rounded-lg bg-gray-200 shadow-md hidden hover:block group-hover:block absolute -right-2 bottom-4 text-left">
...
function ListAmount({ amounts, status }) {
  ...
  if (amounts.length === 0) {
    if (status === FetchJobState.ABORTED) {
      return <span className="text-lg">{t("result_aborted_empty")}</span>;
    }
    if (status === FetchJobState.FINISHED) {
      return <span className="text-lg">{t("result_finished_empty")}</span>;
    }
  }
