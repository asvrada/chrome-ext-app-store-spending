...
export default function StepTwo() {
    ...
    const componentProgress = (
        <div className="p-2">
            <Line className="w-4/5 mx-auto" percent={p} strokeWidth={4} strokeColor="#1698F4"/>
        </div>
    );

    const componentPromptStart = (
        <strong>
            <span>&#9888;&#65039;&#160;</span>{state === FetchJobState.RUNNING
            ? t("heading_running")
    ...
    const componentPromptReset = (
        <div>
            <div>
                {state === FetchJobState.ABORTED ? <strong>&#9940;&#65039;&#160;{t("status_aborted")}</strong> :
        ...
        <div className="m-4 p-2 flex justify-around items-center border-2 rounded-lg border-black">
            ...
            <div className="flex-auto">
                <div className="mb-0.5">
                    {state === FetchJobState.FINISHED || state === FetchJobState.ABORTED ? componentPromptReset : componentPromptStart}
                </div>
                ...
                <div>
                    ...
                    {state === FetchJobState.FINISHED || state === FetchJobState.ABORTED ? componentBtnReset : null}
                </div>
            </div>
        </div>
    );
