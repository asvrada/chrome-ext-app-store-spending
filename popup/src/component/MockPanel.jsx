  ...
  return (
    <div className="border-2 border-yellow-400 fixed bottom-0 left-0 right-0">
      ...
      <div className="flex justify-around">
        <button onClick={() => dispatch(changeState(FetchJobState.NOT_READY))}>
          Not Ready
        </button>
        <button
          onClick={() => dispatch(changeState(FetchJobState.NOT_STARTED))}
        >
          Not Started
        </button>
        <button onClick={() => dispatch(changeState(FetchJobState.RUNNING))}>
          Running
        </button>
        <button
          onClick={() =>
            dispatch(
              handleLoadState({
                ...
                results: {
                  totalAmount: [
                    { currency: "$$$", amount: 9999.99 },
                    { currency: "¥¥¥", amount: 19999.99 },
                  ],
                },
              }),
            )
